...
# Lấy thông tin cấu hình từ các biến môi trường bảo mật trên Railway
...
# Hàm xử lý quét và làm quest tự động ngầm
async def run_auto_quest():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # 1. Gọi API lấy danh sách quest từ tài khoản
            async with session.get("https://discord.com/api/v9/users/@me/quests", headers=headers, timeout=15) as resp:
                if resp.status != 200:
                    return "❌ **LỖI BẢO MẬT:** User Token không hợp lệ hoặc đã bị Discord thu hồi!"
        ...
        # Nếu không có quest nào cần làm thì bỏ qua, không spam kênh chat
        if len(need_action) == 0:
            return None 

        # 2. Tiến hành tự động gửi yêu cầu hoàn thành quest
        processed = 0
        async with aiohttp.ClientSession() as session:
...
# Vòng lặp chạy tự động định kỳ (Cứ mỗi 24 giờ tự quét 1 lần)
@tasks.loop(hours=24)
...
# Lệnh phụ thủ công: Gõ `!checknow` trong kênh nếu muốn bot ép quét ngay lập tức
