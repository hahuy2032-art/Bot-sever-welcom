...
# ==================== LỆNH 2: /AUTOQUES (Quét toàn bộ nhiệm vụ, tối ưu hóa token) ====================
...
async def autoques(interaction: discord.Interaction, token: str):
    ...
    try:
        # Xử lý làm sạch token: xóa khoảng trắng, dấu ngoặc kép thừa nếu lỡ tay copy phải
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as response:
                if response.status != 200:
                    ...
                    await status_msg.edit(content=f"❌ **Lỗi API ({response.status}):** Token không hợp lệ, sai định dạng hoặc đã bị đổi mật khẩu/đăng xuất. Hãy dùng `/gettoken` xem lại cách lấy nhé!")
                    return
