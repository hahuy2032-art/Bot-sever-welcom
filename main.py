...
# 1. Lấy Token của 2 Bot từ biến môi trường trên Railway
...
async def autoquest(interaction: discord.Interaction, token: str):
    await interaction.response.send_message(
        "⚡ **Đã bắt đầu xử lý quest...** (Kiểm tra tin nhắn DM của bạn để xem kết quả)",
        ephemeral=True,
    )
...
# ----------------------------------------------------
# CHẠY SONG SONG CẢ 2 BOT TRONG 1 TIẾN TRÌNH
# ----------------------------------------------------
async def main():
    await asyncio.gather(
        bot1.start(BOT_1_TOKEN),
        bot2.start(BOT_2_TOKEN) if BOT_2_TOKEN and BOT_2_TOKEN.strip() else asyncio.sleep(3600)
    )
