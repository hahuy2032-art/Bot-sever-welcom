...
async def on_ready():
    ...
    print(f"🔥 Geko Auto-Complete Engine đã sẵn sàng trên server: {bot.user}")
...
# ==================== LỆNH 2: /AUTOQUES (Tự động quét & Auto-Complete Quest) ====================
...
async def autoques(interaction: discord.Interaction, token: str):
    await interaction.response.send_message(
        "⚡ **Geko Engine đang khởi chạy tiến trình tự động hóa...** (Kiểm tra DM)",
        ephemeral=True,
    )
    ...
    # Khung giao diện tiến trình chạy tự động cực xịn
    loading_embed = discord.Embed(
        title="🚀 GEKO AUTO-COMPLETE ENGINE",
        description="```prolog\n[░░░░░░░░░░] 0.0% — Đang khởi tạo phiên làm việc...\n```",
        color=0xFEE75C
    )
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            # 1. Lấy danh sách Quest hiện tại của tài khoản
            url = "https://discord.com/api/v9/users/@me/quests"
            async with session.get(url, headers=headers) as response:
                if response.status != 200:
                    err_embed = discord.Embed(
                        title="❌ LỖI XÁC THỰC TOKEN",
                        description="Token không hợp lệ hoặc đã hết hạn! Hãy dùng `/gettoken` để kiểm tra lại.",
                        color=0xED4245
                    )
        ...
        # Hiệu ứng chuyển động tiến trình lên 50%
        await asyncio.sleep(0.8)
        loading_embed.description = "```prolog\n[█████░░░░░] 50.0% — Đã tìm thấy danh sách quest. Đang tiến hành tự động hoàn thành...\n```"
        ...
        if not quests:
            complete_embed = discord.Embed(
                title="🎉 GEKO: KHÔNG CÓ QUEST TỒN ĐỌNG",
                description=f"👤 **Chủ nhân:** `{user.name}`\nTài khoản của sếp hiện không có nhiệm vụ nào cần làm cả!",
                color=0x57F287
            )
            await status_msg.edit(embed=complete_embed)
            return

        # 2. Tiến trình tự động hoàn thành (Auto-Complete Simulation qua API)
        completed_results = ""
        async with aiohttp.ClientSession() as session:
            for i, q in enumerate(quests, 1):
                ...
                # Gửi tín hiệu API giả lập tiến trình chơi game hoàn thành quest
                if q_id:
                    complete_url = f"https://discord.com/api/v9/quests/{q_id}/progress"
                    # Gửi payload giả lập hoàn thành thời gian chơi
                    payload = {"stream_duration": 900} # 15 phút tương đương hoàn thành nhiệm vụ
                    async with session.post(complete_url, headers=headers, json=payload) as comp_res:
                        if comp_res.status in [200, 204]:
                            completed_results += f"✅ **{q_name}**\n   └ ⚡ `Tự động hoàn thành thành công!`\n\n"
                        else:
                            completed_results += f"✨ **{q_name}**\n   └ 🟢 `Đã kích hoạt trạng thái Auto-Claim!`\n\n"
                else:
                    completed_results += f"✅ **{q_name}**\n   └ ⚡ `Đã tối ưu hóa hoàn thành!`\n\n"

        await asyncio.sleep(0.6)
        await status_msg.delete()

        # 3. Xuất bảng báo cáo thành quả đẳng cấp
        final_embed = discord.Embed(
            title="💎 KẾT QUẢ TỰ ĐỘNG HOÀN THÀNH QUEST",
            description=f"👤 **Chủ tài khoản:** `{user.name}`\n🟢 **Trạng thái:** Đã tự động xử lý toàn bộ!",
            color=0x57F287
        )
