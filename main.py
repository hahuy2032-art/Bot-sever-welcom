...
async def autoquest(interaction: discord.Interaction, token: str):
    # PHẢN HỒI LẬP TỨC TRONG 0.1 GIÁY ĐỂ TRIỆT TIÊU LỖI ỨNG DỤNG KHÔNG PHẢN HỒI
    await interaction.response.send_message(
        "⚡ **Đã tiếp nhận lệnh thành công!** Bot đang xử lý ngầm, vui lòng kiểm tra tin nhắn DM.",
        ephemeral=True
    )
    ...
    try:
        ...
        try:
            ...
            embed.add_field(
                name="📌 Trạng thái",
                value=f"🟢 **{total_completed + total_need_action}/{total_quest}** quest đã xong\n❌ **0** lỗi",
                inline=False
            )
            embed.add_field(
                name="🔒 Bảo mật",
