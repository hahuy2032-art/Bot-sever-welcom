...
# ==================== LỆNH 1: /GETTOKEN (Chỉ một mình sếp thấy) ====================
...
async def gettoken(interaction: discord.Interaction):
    ...
    # ephemeral=True giúp tin nhắn phản hồi NÀY CHỈ MỘT MÌNH SẾP THẤY, người khác hoàn toàn không thấy gì!
    secret_guide = (
        f"🔐 **HƯỚNG DẪN LẤY USER TOKEN AN TOÀN CHO {user.name.upper()}**\n\n"
        "Vì lý do bảo mật của Discord, bot không thể tự động lấy trộm token của tài khoản cá nhân. Sếp hãy làm theo 2 bước sau để lấy:\n\n"
        "1️⃣ Mở Discord trên Trình duyệt web (Chrome, Edge... ở chế độ Máy tính / Trang web cho máy tính).\n"
        "2️⃣ Nhấn phím `F12` (hoặc chuột phải chọn **Kiểm tra / Inspect**) -> Chuyển sang tab **Network** (Mạng).\n"
        "3️⃣ Gõ chữ `api` vào ô lọc (Filter) -> Gửi một tin nhắn bất kỳ trên web.\n"
        "4️⃣ Click vào bất kỳ dòng request nào xuất hiện, tìm mục **Headers** -> Kéo xuống tìm dòng **`authorization`**, dãy ký tự dài đằng sau chính là **User Token** của sếp!\n\n"
        "⚠️ *Tuyệt đối không chia sẻ Token này cho bất kỳ ai để tránh bị chiếm đoạt tài khoản!*"
    )
    
    await interaction.response.send_message(secret_guide, ephemeral=True)


# ==================== LỆNH 2: /AUTOQUES (Quét và hiển thị nhiệm vụ) ====================
...
async def autoques(interaction: discord.Interaction, token: str):
    # Tin nhắn phản hồi lệnh này cũng để ephemeral=True để bảo mật tuyệt đối cho token sếp nhập
