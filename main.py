...
async def on_ready():
    try:
        ...
        print(f"🔥 Geko Perfect Engine đã sẵn sàng: {bot.user}")
...
# ==================== LỆNH 2: /AUTOQUES (Phân nhánh thông minh y hệt video) ====================
...
async def autoques(interaction: discord.Interaction, token: str):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status != 200:
                    await status_msg.edit(content="❌ **LỖI:** Token không hợp lệ hoặc đã hết hạn! Hãy dùng `/gettoken` để kiểm tra lại.")
                    return
        ...
        # ---------------- TRƯỜNG HỢP 1: CÒN NHIỆM VỤ CHƯA LÀM ----------------
        ...
        # ---------------- TRƯỜNG HỢP 2: HIỆN LUÔN KẾT QUẢ HOÀN THÀNH ----------------
        result_embed = discord.Embed(
            title="📊 BÁO CÁO TỔNG KẾT QUEST",
            description=f"Chào **{user.name}**, tất cả quest đã được hoàn thành!",
            color=0x57F287
        )
